...
int main() {
    ...
    // 学籍番号とリンクのインデックスを格納するペアのベクターを定義
    std::vector<std::pair<int, int>> indices;

    // linksの各要素（メールアドレス）を順番に処理
    for (size_t i = 0; i < links.size(); ++i) {
        // 'g'の次の位置を見つける（学籍番号の開始位置）
        size_t start = links[i].find('g') + 1;

        // '@'の位置を見つける（学籍番号の終了位置）
        size_t end = links[i].find('@');

        // 学籍番号を抽出し、整数に変換
        // substr(start, end - start)で学籍番号部分の文字列を切り出し、std::stoiで整数に変換
        int id = std::stoi(links[i].substr(start, end - start));

        // 学籍番号（id）と元のリンクのインデックス（i）をペアとしてベクターに追加
        indices.push_back(std::make_pair(id, i));
    }
